...
export const ProblemScrollSection: React.FC = () => {
  ...
  return (
    <section className="relative bg-[#f4f4f6] dark:bg-slate-950 border-b border-slate-300/80 dark:border-slate-800/80 pt-20 sm:pt-28 pb-32">
      ...
      <div className="absolute inset-0 pointer-events-none max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between z-0">
        <div className="w-px h-full bg-slate-300/35 dark:bg-slate-800/40" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="min-h-[50vh] flex flex-col justify-center items-center text-center max-w-4xl mx-auto mb-16 sm:mb-24">
          ...
          >
            {/* Challenge Tag */}
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-mono font-bold uppercase tracking-wider bg-white dark:bg-slate-800 text-[#cf2e46] border border-slate-300 dark:border-slate-700 shadow-2xs">
            ...
            {/* ENLARGED SUBTITLE */}
            <p className="text-base sm:text-lg md:text-xl text-slate-600 dark:text-slate-400 leading-relaxed font-normal max-w-3xl mx-auto">
            ...
            <div className="pt-4 flex flex-col items-center gap-2 text-slate-400">
              <span className="text-[11px] font-mono tracking-widest uppercase font-semibold text-slate-500 dark:text-slate-400">
                Explore the 4 Bottlenecks Below
              </span>
              <div className="w-7 h-7 rounded-full border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-center text-[#cf2e46] shadow-2xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start relative">
          ...
          <div className="lg:col-span-5 space-y-16 sm:space-y-24 py-4">
            ...
            <div className="sticky top-20 z-20 bg-[#f4f4f6]/95 dark:bg-slate-950/95 backdrop-blur-md py-3 border-b border-slate-300/80 dark:border-slate-800/80 mb-6">
              <div className="flex items-center justify-between text-xs font-mono mb-2">
                <span className="text-slate-500 dark:text-slate-400 uppercase font-bold">PROCUREMENT BOTTLENECK</span>
                ...
                {PROBLEM_ITEMS.map((_, idx) => (
                  <div
                    ...
                    className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                      idx === activeStep ? 'bg-[#cf2e46]' : idx < activeStep ? 'bg-slate-800 dark:bg-slate-400' : 'bg-slate-300 dark:bg-slate-800'
                    }`}
                  />
                ))}
              </div>
            ...
            {PROBLEM_ITEMS.map((item, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={`bg-white dark:bg-slate-900 rounded-3xl p-7 sm:p-9 shadow-lg relative overflow-hidden transition-all duration-300 ${
                    isActive
                      ? 'border-2 border-[#cf2e46] shadow-2xl scale-[1.02]'
                      : 'border border-slate-300/80 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-700'
                  }`}
                >
                  {/* Top Red Accent Bar for Active Card */}
                  <div
                    className={`absolute top-0 left-0 right-0 h-1.5 transition-colors duration-300 ${
                      isActive ? 'bg-[#cf2e46]' : 'bg-slate-200 dark:bg-slate-800'
                    }`}
                  />
                  ...
                  <div className="flex items-start justify-between gap-4 mb-6">
                    <div
                      className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black transition-colors ${
                        isActive ? 'bg-rose-50 dark:bg-rose-950/50 text-[#cf2e46]' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                      }`}
                    >
                    ...
                    <div className="flex flex-col items-end">
                      <span className="font-mono font-bold text-xs px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
                        STEP {item.step} / 04
                      </span>
                  ...
                  {/* Impact Tag */}
                  <div className="p-4 rounded-2xl bg-[#f4f4f6] dark:bg-slate-850/80 border border-slate-200 dark:border-slate-800 flex items-center gap-3">
          ...
          <div className="lg:col-span-7 lg:sticky lg:top-24 self-start py-4">
            <div className="relative rounded-3xl border-2 border-slate-300/90 dark:border-slate-800 bg-slate-950 shadow-2xl overflow-hidden aspect-[16/10] sm:aspect-[16/9] group">
