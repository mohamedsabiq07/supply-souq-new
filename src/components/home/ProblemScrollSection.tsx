...
export const ProblemScrollSection: React.FC = () => {
  ...
  // Extended 720vh scroll track gives generous reading time for every stage
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ['start start', 'end end']
  });

  // Calculate phase and active card with ample spacing
  useEffect(() => {
    const unsubscribe = scrollYProgress.on('change', (latest) => {
      // Phase 1: Enlarged Headline & Subtitle Hero (0.00 to 0.20)
      // Phase 2: Persistent Video + Sequential Cards (0.20 to 1.00)
      if (latest < 0.20) {
        setIsPhase1(true);
  ...
  return (
    ...
    >
      ...
      {/* Sticky Full-Viewport Storytelling Stage */}
      <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col justify-center px-4 sm:px-6 lg:px-8 z-10">
        
        {/* ========================================================================= */}
        {/* PHASE 1: ENLARGED HEADLINE & SUBTITLE HERO STAGE                          */}
        {/* Renders ONLY when isPhase1 is true (Zero visual overlap with Phase 2)     */}
        {/* ========================================================================= */}
        {isPhase1 && (
          <div className="w-full max-w-5xl mx-auto flex flex-col items-center justify-center text-center px-4 sm:px-6 py-12 transition-opacity duration-300">
            <div className="space-y-6">
              ...
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-mono font-bold uppercase tracking-wider bg-white text-[#cf2e46] border border-slate-300 shadow-2xs">
                <AlertTriangle className="w-3.5 h-3.5 text-[#cf2e46]" />
                <span>[SS®—CHALLENGE] THE TRADITIONAL PROCUREMENT BOTTLENECK</span>
              </div>

              {/* ENLARGED HEADLINE */}
              <h2 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-slate-950 tracking-tight leading-[1.08] max-w-4xl mx-auto">
              ...
              {/* Scroll Cue to encourage user to scroll down */}
              <div className="pt-8 flex flex-col items-center gap-2 text-slate-400">
                <span className="text-[11px] font-mono tracking-widest uppercase font-semibold text-slate-500">
                  Scroll down slowly to explore bottlenecks
                </span>
                <div className="w-8 h-8 rounded-full border border-slate-300 bg-white flex items-center justify-center text-slate-600 shadow-xs animate-bounce">
                  <ArrowDown className="w-4 h-4 text-[#cf2e46]" />
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Renders ONLY when isPhase1 is false (Zero visual overlap with Phase 1)    */}
        {/* ========================================================================= */}
        {!isPhase1 && (
          <div className="w-full max-w-7xl mx-auto flex flex-col justify-center h-full max-h-[92vh] py-4 transition-opacity duration-300">
            {/* Top Bar Header */}
            <div className="flex items-center justify-between pb-4 sm:pb-5 border-b border-slate-300/70 mb-4 sm:mb-6">
              <div className="flex items-center gap-3">
                <span className="w-2.5 h-2.5 rounded-full bg-[#cf2e46] animate-pulse" />
                <h3 className="text-base sm:text-xl md:text-2xl font-black text-slate-950 tracking-tight">
                  Material procurement shouldn't take days of follow-ups.
                </h3>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 items-center">
              ...
              <div className="lg:col-span-5 flex flex-col justify-center space-y-4">
                ...
                <div className="flex items-center gap-2 mb-2">
                  {PROBLEM_ITEMS.map((item, idx) => (
                    <div
                      ...
                      style={{
                        backgroundColor:
                          idx === activeCardIndex ? '#cf2e46' : idx < activeCardIndex ? '#1e293b' : '#cbd5e1'
                      }}
                    />
                  ))}
                </div>
                ...
                <div className="relative min-h-[300px] sm:min-h-[330px] flex items-center">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-4 mb-5">
                            <div className="w-13 h-13 rounded-2xl bg-rose-50 text-[#cf2e46] flex items-center justify-center font-black shadow-xs">
                              <IconComponent className="w-6 h-6 text-[#cf2e46]" />
                            </div>
