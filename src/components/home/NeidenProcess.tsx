...
export const NeidenProcess: React.FC<NeidenProcessProps> = ({ onStartRFQ }) => {
  ...
  return (
    <section className="py-24 md:py-32 bg-[#040908] text-white border-b border-white/10 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((s, idx) => (
            <motion.div
              ...
              className="p-8 rounded-3xl bg-white/[0.02] border border-white/10 hover:border-emerald-500/30 transition-all duration-300 flex flex-col justify-between space-y-8 group"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-4xl sm:text-5xl font-mono font-black text-slate-600 group-hover:text-emerald-400 transition-colors">
                    {s.step}
                  </span>
                ...
                <div className="inline-block px-2.5 py-1 rounded bg-white/5 border border-white/10 text-[10px] font-mono text-emerald-400 font-bold">
                  {s.tag}
                </div>
        ...
        {/* Bottom CTA bar */}
        <div className="mt-14 p-6 sm:p-8 rounded-2xl bg-emerald-950/20 border border-emerald-500/30 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
          ...
          <button
            onClick={onStartRFQ}
            className="px-6 py-3 bg-emerald-400 hover:bg-emerald-300 text-slate-950 rounded-full font-black text-xs font-mono transition-all shadow-glow-mint flex items-center gap-2 shrink-0 cursor-pointer"
          >
