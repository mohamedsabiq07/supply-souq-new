...
export const MarketTicker: React.FC = () => {
  ...
  return (
    <div className="bg-[#050505] border-b border-[#cf2e46]/20 text-white overflow-hidden select-none py-1.5 px-3 relative z-30 font-sans text-[11px] font-medium tracking-wide">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        {/* Live Badge Indicator */}
        <div className="flex items-center gap-2 shrink-0 bg-[#2a050dcc] text-[#ffb3bf] px-2.5 py-0.5 rounded-full border border-[#cf2e46]/35 shadow-glow-red">
              ...
              return (
                <div key={`${item.id}-${idx}`} className="inline-flex items-center gap-2 text-slate-300">
                  ...
                  {item.source === 'dynamic_platform' && (
                    <span className="text-[9px] font-mono px-1 py-0.2 rounded bg-rose-950 text-rose-300 border border-rose-800/40">
                      LIVE RFQ
                    </span>
                  )}
                  {item.source === 'financial_feed' && (
                    <span className="text-[9px] font-mono px-1 py-0.2 rounded bg-neutral-950 text-neutral-300 border border-neutral-800/80">
                      LME
                    </span>
                  )}
