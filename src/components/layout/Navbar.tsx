...
export const Navbar: React.FC<NavbarProps> = ({ currentView, setCurrentView }) => {
  ...
  return (
    <header className={`sticky top-0 z-40 transition-colors duration-300 ${isHome ? 'bg-[#08090D]/85 backdrop-blur-xl border-b border-white/10 text-white' : 'bg-white/95 backdrop-blur-xl border-b border-slate-200 text-slate-900'} shadow-2xs`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-[#cf2e46] shadow-xs group-hover:scale-105 transition-transform ${isHome ? 'bg-rose-950/80 border border-rose-500/30' : 'bg-rose-50 border border-rose-200'}`}>
                <Layers className="w-5 h-5 text-[#cf2e46]" />
              </div>
              <div>
                <div className="flex items-center gap-1.5">
                  <span className={`font-extrabold text-lg tracking-tight ${isHome ? 'text-white' : 'text-slate-900'}`}>
                    Supply<span className="text-[#cf2e46]">Souq</span>
                  </span>
                </div>
                <span className={`text-[10px] font-semibold block leading-none ${isHome ? 'text-slate-400' : 'text-slate-500'}`}>
            ...
            <nav className={`hidden md:flex items-center gap-1 text-xs font-semibold ${isHome ? 'text-slate-300' : 'text-slate-600'}`}>
              <button
                onClick={() => setCurrentView('home')}
                className={`px-3 py-2 rounded-lg transition-colors cursor-pointer ${
                  currentView === 'home'
                    ? (isHome ? 'bg-white/10 text-white font-bold border border-white/20' : 'bg-slate-100 text-slate-950 font-bold border border-slate-200')
                    : (isHome ? 'hover:bg-white/10 hover:text-white' : 'hover:bg-slate-50 hover:text-slate-950')
                }`}
              >
                Home
              </button>
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg transition-colors cursor-pointer ${isHome ? 'hover:bg-white/10 hover:text-white' : 'hover:bg-slate-50 hover:text-slate-950'}`}
              >
                Pricing
              </button>
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg transition-colors flex items-center gap-1 font-bold cursor-pointer ${
                  currentView === 'invoice-audit'
                    ? 'bg-rose-50 text-[#cf2e46] border border-rose-200 shadow-2xs'
                    : (isHome ? 'bg-rose-950/60 hover:bg-rose-900/80 text-rose-300 border border-rose-500/30' : 'bg-rose-50/70 hover:bg-rose-100 text-[#cf2e46] border border-rose-200')
                }`}
              >
            ...
            {!isAuthenticated ? (
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`px-3 py-2 text-xs font-bold transition-colors cursor-pointer ${isHome ? 'text-slate-200 hover:text-white' : 'text-slate-700 hover:text-[#cf2e46]'}`}
                >
                  Log in
                </button>
                <button
